...
func (alarm *SlackAlarm) sendToSlack(message string) {
...
func (alarm *SlackAlarm) NotifyError(monitor monitors.Monitor) {
	alarm.sendToSlack(`{ "text": "` + monitor.Name() + ` has raised an alarm" }`)
}

func (alarm *SlackAlarm) NotifyRestore(monitor monitors.Monitor) {
	alarm.sendToSlack(`{ "text": "` + monitor.Name() + ` is back to normal" }`)
}
